...
class LineaRepository:
    ...
    @staticmethod
    def createLinea(db, nameLinea, idDepartment):
        cursor = None
        ...
        try:
            ...
            query = """
                INSERT INTO turnos_linea(nameLinea, idDepartment)
                VALUES (%s, %s)
                """
            cursor.execute(query, (nameLinea, idDepartment,))
            ...
            return {"mensaje": f"Línea creada correctamente. ID: {newLinea['idLinea']}, Nombre: {newLinea['nameLinea']}, idDepartment: {newLinea['idDepartment']}"}

        
        except Exception as ex:
            db.connection.rollback()
            
            return {"error": f"No se pudo crear linea en repositorio: {str(ex)}"}
    ...
    @staticmethod
    def updateLinea(db, idLinea, nameLinea, idDepartment):
        cursor = None
        ...
        try: 
            ...
            query = """
                UPDATE turnos_linea SET nameLinea = %s, idDepartment = %s
                WHERE idLinea = %s
                """
            
            cursor.execute(query, (nameLinea, idDepartment, idLinea))
            ...
            return {"mensaje": f"Línea modificada correctamente. ID: {editedLinea['idLinea']}, Nombre: {editedLinea['nameLinea']}, idDepartment: {editedLinea['idDepartment']}"}

        except Exception as ex:
            db.connection.rollback()
                        
            return {"error": f"No se pudo modificar linea en repositorio: {str(ex)}"}
